...
const Signup = ({ setView }) => {
  ...
  function handlePassword(event) {
    ...
    if (!new_pass.match(lowerCase)) {
      ...
      setErrorMessage("Password length should be more than 10 characters.");
    } else {
      setErrorMessage("");
    }
  }
  ...
  function handleNumber(event) {
    ...
    if (!new_num.match(numbers)) {
      setErrorMessage2("Phone Number should contain numbers only!");
    } else if (new_num.length < 10) {
      setErrorMessage2("Phone Number length should be more than 10 characters.");
    } else {
      setErrorMessage2("");
    }
  }
  ...
  const handleClick = (event) => {
    ...
    if (!firstname || !lastname || !email || !number || !password || !password2 || errorMessage2 !== "" || errorMessage !== "") {
      setErrorMessage5("Please fill all the fields correctly!");
    } else if (!validateEmail(email)) {
      setErrorMessage5("Please enter a valid email address!");
    ...
    <div className="relative min-h-screen flex items-center justify-center">
      ...
      <div className={`relative bg-white shadow-lg p-8 max-w-md w-full mt-6 mb-4 ${window.innerWidth <= 768 ? 'rounded-x' : 'rounded-3xl'}`}>
        ...
        <form onSubmit={handleClick}>
          ...
          <button type="submit" className="w-full bg-blue-400 text-white py-2 rounded-3xl font-bold">Sign Up</button>
